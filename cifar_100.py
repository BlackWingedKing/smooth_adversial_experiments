...
if __name__ == '__main__':
    ...
    if args.mode == "cln":
        flag_advtrain = False
        nb_epoch = 10
        model_filename = "cifar_resnet50_clntrained.pt"
    elif args.mode == "adv":
        flag_advtrain = True
        nb_epoch = 90
        model_filename = "cifar_resnet50_advtrained.pt"
    else:
        raise

    # train and test loaders for cifar 100 must be included here
    ...
    if flag_advtrain:
        ...
        adversary = PGDAttack(
            model, loss_fn=nn.CrossEntropyLoss(reduction="sum"), eps= 4.0/255.0,
            nb_iter=1, eps_iter= 4.0/255.0, rand_init=True, clip_min=0.0,
